...
async fn main() {
    ...
    let load_result: Result<Resources, String> = async {
        let json_data = load_string("assets/timeline.json").await
            .map_err(|e| format!("Timeline missing: {:?}", e))?;
        
        let data = AvatarData::from_json(&json_data)
            .map_err(|e| format!("Timeline corrupt: {:?}", e))?;
        
        let texture = load_texture("assets/face.jpg").await
            .map_err(|e| format!("Texture missing: {:?}", e))?;
    ...
    match load_result {
        Ok(res) => state = AppState::Running(res),
        Err(e) => {
            #[cfg(target_family = "wasm")] macroquad::logging::error!("{}", &e);
    ...
    loop {
        match state {
            ...
            AppState::Error(ref e) => {
                ...
                // Only draw text if we absolutely have to, or just log it
                #[cfg(not(target_family = "wasm"))]
